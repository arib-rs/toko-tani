                        ...
                        <form id="form-data">
                            <div class="box">
                                    ...
                                    <table id="dt-nota" class="table table-striped table-bordered" cellspacing="0" width="100%" style="margin-bottom: 20px">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="text-center" style="width: 13%">No. Batch</th>
                                ...
                                <div class="box-footer">
                                    ...
                                    <div id="boxppn" class="row form-group" style="display:none">
                                        <label class="col-md-4 col-md-offset-4 col-xs-4 text-right">Nilai PPN</label>
                                        <div class="col-md-4 col-xs-8 text-right">
                                            <span id="nilaippn" style="font-weight: bold">Rp. 0</span>
                                        </div>
                                    </div>
                                    <div id="boxtotalkeseluruhan" class="row form-group" style="display:none">
                                        <label class="col-md-4 col-md-offset-4 col-xs-4 text-right">Total Keseluruhan</label>
                                        <div class="col-md-4 col-xs-8 text-right">
                                            <span id="totalkeseluruhan" style="font-weight: bold">Rp. 0</span>
                                        </div>
                                    </div>
                                    <div class="row form-group">
...
<script type="text/javascript">
    $(document).ready(function() {
        $('#ppn').click(function() {
            if ($('#ppn').is(":checked")) {
                $('#boxppn').css("display", "block");
                $('#boxtotalkeseluruhan').css("display", "block");
            } else {
                $('#boxppn').css("display", "none");
                $('#boxtotalkeseluruhan').css("display", "none");
            }
        })
        ...
        $('#btn-reset').click(function() {
            ...
            $('#form-data').find('#nilaippn').html('Rp. 0');
            $('#form-data').find('#totalkeseluruhan').html('Rp. 0');
        ...
        if (tbody.find('tr#' + id).length > 0) {
            ...
            var nama = b.data('nama'),
                ...
                hb = b.data('hb');
            tbody.append(
                '<tr id="' + id + '"><td class="text-center">' + kode + '</td><td class="text-center"><input type="hidden" name="id[]" value="' + id + '"><div class="form-group"><input type="text" id="nobatch" name="nobatch[]" class="form-control input-sm text-right" autocomplete="off" value=""></div></td><td><div class="form-group"><input type="text" id="qty" name="qty[]" class="form-control input-sm text-right" autocomplete="off" value="" maxlenght="15"></div></td><td class="text-center">' + satuan + '</td><td>' + nama + '<i id="btn-remove" class="fa fa-trash text-danger pull-right" style="cursor: pointer; line-height: 21px"></i></td><td><input id="hb" class="form-control text-right" type="text" name="hb[]" value="' + hb + '"></td><td id="total" class="text-right"></td><td><div class="form-group"><input type="text" class="form-control kadaluarsa" id="kadaluarsa" name="kadaluarsa[]"></div></td></tr>');


        }
        tbody.find('.kadaluarsa').datepicker({
            format: 'dd/mm/yyyy',
        });
        ...
        tbody.find('tr').each(function() {
            if ($(this).find('input#qty').val() != '' && $(this).find('input#hb').val() != '') {
                ...
                subtotal += _hb * _qty;
                nilaippn += _hb * _qty * 10 / 100;
                totalkeseluruhan += _hb * _qty + (_hb * _qty * 10 / 100);

            }
        });
        $('#form-data').find('#subtotal').html('Rp. ' + subtotal.formatMoney(2, '.', ','));
        $('#form-data').find('#nilaippn').html('Rp. ' + nilaippn.formatMoney(2, '.', ','));
        $('#form-data').find('#totalkeseluruhan').html('Rp. ' + totalkeseluruhan.formatMoney(2, '.', ','));
