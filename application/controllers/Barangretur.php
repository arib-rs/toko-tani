        ...
        <?php
            ...
            function ssplist_($supplier = '0', $gudang = '0')
            {
                ...
                foreach ($rResult as $aRow) {
                    ...
                    $aksi = '<a id="btn-add" class="btn btn-xs btn-success" data-toggle="tooltip" data-placement="left" title="Tambah Barang" data-id="' . $aRow['prd_id'] . '" data-nama="' . $aRow['prd_nama'] . '" data-satuan="' . $aRow['prd_satuan'] . '" data-kode="' . $aRow['prd_kode'] . '" data-hb="' . $aRow['prd_hargabeli'] . '" data-stok="' . $aRow['stok'] . '" data-kadaluarsa="' . $aRow['stk_kadaluarsa'] . '" data-nobatch="' . $aRow['stk_nobatch'] . '"><i class="fa fa-plus"></i></a>';
                    $row = array(($no + 1), $kode . $aRow['prd_nama'], $this->ascfunc->nf_($aRow['stok']), $aRow['prd_satuan'], $aRow['stk_kadaluarsa'], $aksi);
                    $output['aaData'][] = $row;
                    $no++;
                }
