<div class="content-wrapper">
    <section class="content-header">
        <h1>
            Daerah
            <small>Menu untuk melakukan tambah, edit dan hapus data daerah kabupaten/kota</small>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        <div class="box">
            <div class="box-header with-border">
                <h3 class="box-title">Form Daerah</h3>
    ...
    $(document).ready(function () {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                form.find('#id').val(result.dae_id);
                form.find('#nama').val(result.dae_nama);
                form.find('#nama').focus();
                b.removeAttr('disabled');
                i.removeClass().addClass(cls);
            },
        ...
        bootbox.confirm("Apakah anda akan menghapus data tersebut?", function (vars) {
            if (vars) {
                var tStart = Date.now();
                $.ajax({
                    url: '<?= base_url('Daerah/delete_') ?>/' + id,
                    dataType: 'JSON',
                    async: false,
                    beforeSend: function () {
                        b.attr('disabled', 'disabled');
