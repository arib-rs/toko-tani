...
class Penjualan extends CI_Controller {
    ...
    function cari_() {
        ...
        $gudang = $this->session->userdata('log_gudang');
        $sql = $this->db->select('m_produk.*, SUM(stk_jumlah) as stok')
                ->from('m_produk')
        ...
        foreach ($sql->result_array() as $val) {
            $data['content'] = $val['prd_nama'] . ' [' . $val['prd_satuan'] . ']';
            $data['id'] = $val['prd_id'];
    ...
    function ssplist_($kategori = 0) {
        ...
        $gudang = $this->session->userdata('log_gudang');
        $aColumns = array('prd_kode', 'prd_nama', 'stok', 'prd_satuan', 'prd_id');
...
// individual column filtering
        ...
        foreach ($rResult as $aRow) {
            ...
            $aksi = '<a id="btn-add" class="btn btn-xs btn-success" data-toggle="tooltip" data-placement="left" title="Tambah Barang" data-id="' . $aRow['prd_id'] . '" data-nama="' . $aRow['prd_nama'] . '" data-satuan="' . $aRow['prd_satuan'] . '" data-kode="' . $aRow['prd_kode'] . '" data-hb="' . $aRow['prd_hargabeli'] . '" data-hargaecer="' . $aRow['prd_hargaecer'] . '" data-hargagrosir="' . $aRow['prd_hargagrosir'] . '" data-hargamember="' . $aRow['prd_hargamember'] . '" data-hargakhusus="' . $aRow['prd_hargakhusus'] . '" data-stok="' . $aRow['stok'] . '"><i class="fa fa-plus"></i></a>';
